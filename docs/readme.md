...
## Быстрая установка для Keenetic (Entware)

Для Keenetic рекомендуется Entware и запуск через отдельный инсталлятор, который открывает меню.
Установка/обновление выполняется из меню по выбору.
